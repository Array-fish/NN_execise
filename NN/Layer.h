...
private:
	...
	vector<double> sum_dL_dx_for_this;                        // この層が更新に使うための後の層から持ってきたやつ
	vector<double> sum_dL_dx_for_before;             // 前の層が更新に使うやつ 
	vector<vector<double>> sum_variation_for_patch; // パッチ学習のときにデータ全部の誤差をためとくやつ
	// func
...
public:
	...
	void set_sum_dL_dx_for_this(const vector<double>& dL_dx);         // 後ろからとってきた誤差を取り込む用
	void calc_dL_dx_for_before();                                   // 前の層が使う誤差dL/dYを作る　重み更新の前に使ってね
	void update_weights();                               // バイアスに対するものを含む重みの更新　
	vector<double> get_sum_dL_dx_for_before();               // 前の層が使う誤差dL/dYを渡すとき用
	void pool_variation();                                  // 誤差を蓄積する
	void update_weights_for_patch(int data_size);        // パッチ学習でsum_variation_for_patchを使って重みを更新する
...
// 後ろからとってきたのを突っ込むよう
inline void layer::set_sum_dL_dx_for_this(const vector<double>& sum_dL_dx) {
	this->sum_dL_dx_for_this = sum_dL_dx;
}
inline vector<double> layer::get_sum_dL_dx_for_before() {
	return sum_dL_dx_for_before;
}
inline void layer::reset_weights_variation() {
	this->sum_variation_for_patch = vector<vector<double>>(num_rows, vector<double>(num_inputs + 1, 0));// 初期化
}
#endif LAYER_H
